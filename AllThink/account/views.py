...
def fileManager(request):
	formFile = FormFile()
	if request.FILES:
		formFile = FormFile(request.POST, request.FILES)
	...
	return render_to_response('account/file_manager.html',{'username':request.user, 'profile':getProfile(request.user), 'files':allFiles, 'upload':formFile})
...
def yourLessons(request):
	yourLessons = []
	user = request.user
	return render_to_response('account/your_lessons.html',{'username': request.user, 'profile':getProfile(request.user), 'yourLessons': user.lesson_set.all()})
...
def joinLesson(request, lessonID, keyWord = ''):
	...
	lesson.save()
	return  render_to_response('success.html',{'username': request.user, 'profile':getProfile(request.user)})
...
def lessonsJoined(request):
	...
	for i in user.lessonsjoined.all():
		if i.user != request.user:
			lessons.append(i)
	return render_to_response('account/lessons_joined.html',{'username': request.user, 'profile':getProfile(request.user), 'lessons':lessons})

def profile(request):
	profile = getProfile(request.user)
	formProfile = FormProfile(initial={'information': profile.information})
	if request.POST:
		formProfile = FormProfile(request.POST, request.FILES)
		if formProfile.is_valid():
			profile.information = formProfile.cleaned_data['information']
			profile.save()
			if request.FILES:
				if profile.file:
					profile.file.delete()
				profile.uploadFile(request)
				profile.save()
	return render_to_response('account/profile.html',{'username': request.user, 'profile':getProfile(request.user), 'profile':profile, 'formProfile':formProfile})